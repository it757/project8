...
optimal.cp <- my.tree2$cptable[,"CP"][which.min(my.tree2$cptable[,"xerror"])]  # get the value from the CP column that corresponds to the minimum value from the xerror column
...
my.rf <- randomForest(ADOPT_SUB_CLASS~HCP_Yr_Experience+Eigenvector_Centrality+HCP_Affiliation_Count+Degree+In_Degree+Out_Degree,
                      data=cr1$build, na.action=na.exclude, ntree=50, nodesize=10, ntry=4, keep.forest=TRUE, importance=TRUE)
my.rf
